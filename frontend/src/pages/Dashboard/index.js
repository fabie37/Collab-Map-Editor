...
export default function Dashboard({ history }) {
    ...
    const subscriptionRequestHandler = async (event) => {
        try {
            await api.post(`/subscription/${event.id}`, {}, { headers: { user } })
            setSuccess(true)
    ...
    const acceptEventHandler = async (eventId) => {
        try {
            await api.post(`/subscription/${eventId}/approvals`, {}, { headers: { user } })
    ...
    const rejectEventHandler = async (eventId) => {
        try {
            await api.post(`/subscription/${eventId}/rejections`, {}, { headers: { user } })
            setEventRequestSuccess(true)
            ...
            <ul className="notifications">
                {eventsRequest.map(request => {
                    return (
                        <li key={request._id}>
                            <div>
                                <strong>{request.user.email} </strong> is requesting to subscribe to your Event <strong>{request.event.title}</strong>
                            </div>
                            <ButtonGroup>
            ...
            <ul className="events-list">
                {events.map(event => (
                    < li key={event._id} >
                        ...
                        <Button color="primary" onClick={() => subscriptionRequestHandler(event)}>Subscription Request</Button>
                    </li>
                ))}
            </ul>
            {
